...
namespace OmegaPlayer.Features.Library.Views
{
    public partial class ArtistsView : UserControl
    {
        ...
        private void OnLoaded(object sender, RoutedEventArgs e)
        {
            ...
            _scrollViewer = this.FindControl<ScrollViewer>("ArtistsScrollViewer");
        }
        ...
        {
            if (_isDisposed || _scrollViewer == null || _artistsItemsRepeater == null) return;
            if (!(DataContext is ArtistsViewModel viewModel)) return;
        ...
        {
            if (_artistsItemsRepeater == null) return null;

            try
            {
                var containers = _artistsItemsRepeater.GetVisualChildren().OfType<Control>().Where(c => c.DataContext != null).Take(5).ToList();
        ...
        protected override void OnAttachedToVisualTree(VisualTreeAttachmentEventArgs e)
        {
            ...
            try
            {
                ...
                if (mainView != null)
                {
                    ...
                    _windowResizeHandler?.RegisterArtistView(this);
                }
            }
            catch (Exception ex)
            {
                _errorHandlingService?.LogError(ErrorSeverity.NonCritical, "Error registering ArtistsView with WindowResizeHandler", ex.Message, ex, false);
            }
        }
        ...
        private void OnDetachedFromVisualTree(object sender, VisualTreeAttachmentEventArgs e)
        {
            try
            {
                _windowResizeHandler?.UnregisterArtistView(this);
                _windowResizeHandler = null;
            }
            catch (Exception ex)
            {
                _errorHandlingService?.LogError(ErrorSeverity.NonCritical, "Error unregistering ArtistsView from WindowResizeHandler", ex.Message, ex, false);
            }
        }
        ...
        public void OnResizeStarted()
        {
            ...
            {
                _errorHandlingService?.LogError(ErrorSeverity.NonCritical, "Error handling resize start in ArtistsView", ex.Message, ex, false);
            }
        }
        ...
        public void OnResizeCompleted()
        {
            ...
            {
                _errorHandlingService?.LogError(ErrorSeverity.NonCritical, "Error handling resize completion in ArtistsView", ex.Message, ex, false);
            }
        }
